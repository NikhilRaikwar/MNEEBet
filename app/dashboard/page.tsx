      ...
      <main className="flex-1 container mx-auto px-6 py-20 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-start justify-between gap-12 mb-24">
          <div className="space-y-6 max-w-2xl">
            ...
            <h1 className="text-6xl md:text-8xl font-black tracking-tighter text-white font-display leading-[0.85] uppercase">
              THE <br />
        ...
        {/* Stats Grid */}
        <section className="mb-32 grid grid-cols-1 md:grid-cols-3 gap-0 border-2 border-white/5 divide-y-2 md:divide-y-0 md:divide-x-2 divide-white/5 bg-black/40 backdrop-blur-xl">
          {stats.map((stat, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0 }}
        ...
        <section id="marketplace" className="space-y-12">
            ...
            <div className="flex w-full md:w-auto gap-4">
              <div className="relative flex-1 md:w-96 group">
                ...
                <Input
                  ...
                  className="h-14 pl-14 pr-6 bg-white/5 border-2 border-white/10 text-white placeholder:text-zinc-800 focus:border-primary/50 focus:ring-0 rounded-none transition-all font-mono uppercase text-xs font-bold tracking-widest"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
              </div>
              <Button variant="outline" className="h-14 w-14 rounded-none border-2 border-white/10 bg-white/5 hover:border-primary hover:text-primary transition-all">
          ...
          <Tabs defaultValue="all" onValueChange={setStatusFilter} className="space-y-12">
            <TabsList className="bg-transparent h-auto p-0 flex flex-wrap gap-4">
              {['all', 'open', 'active', 'resolved'].map((tab) => (
                <TabsTrigger
                  key={tab}
                  value={tab}
                  className="data-[state=active]:bg-primary data-[state=active]:text-black border-2 border-white/10 data-[state=active]:border-black px-8 py-3 text-[10px] font-black uppercase tracking-[0.2em] rounded-none transition-all shadow-none data-[state=active]:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]"
                >
                  {tab}
                </TabsTrigger>
              ))}
