...
import { Trophy, Menu, X, Rocket, LayoutDashboard, PlusCircle, BarChart3, Terminal } from 'lucide-react';
...
export function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 w-full transition-all duration-300 ${
      scrolled ? 'h-16 bg-background/90 backdrop-blur-xl border-b-2 border-primary/20' : 'h-24 bg-transparent'
    }`}>
      <div className="container mx-auto h-full flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-6">
          <div className="hidden md:flex items-center gap-6 font-mono">
            ...
            <Button
              className="bg-primary text-black hover:bg-primary/90 font-black rounded-none px-6 h-10 border-2 border-black shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(255,255,255,1)] active:translate-x-0 active:translate-y-0 active:shadow-none uppercase text-xs tracking-widest"
              asChild
            >
              <Link href="/create">
                INIT_BATTLE
          ...
          <Button
            variant="ghost"
            size="icon"
            className="lg:hidden text-primary rounded-none border-2 border-primary/20 h-10 w-10"
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-40 lg:hidden bg-background/95 backdrop-blur-2xl flex flex-col p-10 pt-32"
          >
            ...
            <div className="mt-auto pt-10 border-t-2 border-white/5 flex flex-col gap-6">
              ...
              <Button className="w-full h-16 bg-primary text-black font-black text-xl rounded-none border-2 border-black uppercase" asChild onClick={() => setIsMenuOpen(false)}>
                <Link href="/create">HOST_BET</Link>
              </Button>
            </div>
          </motion.div>
        )}
